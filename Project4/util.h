...
typedef struct CPU_State_Struct {
	...
	uint32_t PIPE_STALL[PIPE_STAGE];
        //uint32_t CACHE_STALL[PIPE_STAGE];
	...
	unsigned char ID_EX_DEST;       // save destination register (RD)

	//EX_MEM_latch
	uint32_t EX_MEM_NPC;
        unsigned char EX_MEM_REG;       // REG TO WRITE IN
	uint32_t EX_MEM_ALU_OUT;
        uint32_t EX_MEM_DEST;           // MEM TO WRITE OR READ
	uint32_t EX_MEM_W_VALUE;
	uint32_t EX_MEM_BR_TARGET;
	uint32_t EX_MEM_BR_TAKE;
        uint32_t EX_MEM_BRANCH_DEST;
        uint32_t EX_MEM_CHOICE;         // 00: PASS / 01: WRITE MEM / 10: READ MEM

	//MEM_WB_latch
	uint32_t MEM_WB_NPC;
        uint32_t MEM_WB_REG;            // REG TO WRITE
	uint32_t MEM_WB_ALU_OUT;
	uint32_t MEM_WB_MEM_OUT;
	uint32_t MEM_WB_BR_TAKE;
        uint32_t MEM_WB_ALU;            // 0 IF WRITE ALU_OUT 1 IF WRITE MEM_OUT
	unsigned char MEM_WB_DEST;

        //MEM_STATE
        int CACHE_STALL;    // variable to check stall
	...
	unsigned char MEM_WB_FORWARD_REG1;
        unsigned char MEM_WB_FORWARD_REG2;
	uint32_t EX_MEM_FORWARD_VALUE;
	uint32_t MEM_WB_FORWARD_VALUE1;
        uint32_t MEM_WB_FORWARD_VALUE2;
        short EX_MEM_WRITE;
        short MEM_WB_WRITE;
