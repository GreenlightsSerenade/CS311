...
void process_instruction(){
    ...
    if (CYCLE_COUNT && CURRENT_STATE.CACHE_STALL == 0)
    ...
    */
    ...
    /* CACHE STALL CHECK */
    if (CURRENT_STATE.CACHE_STALL != 0) {
        // if stalling because of cache, just return
        if (DEBUG) printf("STALLING.... -> %d\n", CURRENT_STATE.CACHE_STALL);
        CURRENT_STATE.CACHE_STALL --;
    ...
    if (CURRENT_STATE.PIPE_STALL[EX_STAGE] == 0 && CURRENT_STATE.PIPE[EX_STAGE]) {
        ...
        if (CURRENT_STATE.EX_MEM_CHOICE == 1) {		// SW process
            BUF.CACHE_STALL = 29;
            writeCache(CURRENT_STATE.EX_MEM_DEST, CURRENT_STATE.EX_MEM_W_VALUE);
            ...
            if (hit_flag == 0) {
                // stall if miss
                BUF.CACHE_STALL = 30;
                if (DEBUG) printf("STALL STARTED **********SW**********\n");
            }
            else BUF.CACHE_STALL = 0;
        }
        else if (CURRENT_STATE.EX_MEM_CHOICE == 2) {	// LW process
            //BUF.MEM_WB_MEM_OUT = mem_read_32(CURRENT_STATE.EX_MEM_DEST);
            BUF.MEM_WB_MEM_OUT = readCache(CURRENT_STATE.EX_MEM_DEST);
            BUF.MEM_WB_ALU = TRUE;
	    ...
	    /* Forwarding preparation */
            ...
            if (hit_flag == 0) {
                // stall if miss
                BUF.CACHE_STALL = 30;
                if (DEBUG) printf("STALL STARTED **********LW*********\n");
            }
            else BUF.CACHE_STALL = 0;
        }
        else {
